    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if self.ant.__class__ == QueenAnt and self.ant.first:
                    return
                ...
                if insect.__class__ == QueenAnt and insect.first:
                    return
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 8
        ant = self.place.ant
        if ant == None or not ant.blocks_path:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for i in range(self.min_range):
            ...
            counter += 1
        while len(location.bees) == 0 and location.entrance is not hive and counter < self.max_range:
            location = location.entrance
            counter += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """ IMPORTANT NOTE TO THE COMPOSITION GRADER:
                We changed Insect.reduce_armor to Ant.reduce_armor as you suggested!
                However, the reason that we did not place Ant.reduce_armor before the if statement is
                because the Ant's reduce_armor method completely removes the ant when it is dead
                (armor reaches zero). When we tried to reduce armor before the if statement,
                the insect became nonexistent if updated armor <= 0, and future calls to "self" within
                the if statement did not work.
                Thanks! :)
        """
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.first:
            ...
            while location:
                if location.ant and location.ant not in self.doubled_ants:
                    location.ant.damage *= 2
                    self.doubled_ants.append(location.ant)
                if isinstance(location.ant, BodyguardAnt) and location.ant.ant and location.ant.ant not in self.doubled_ants:
                    location.ant.ant.damage *= 2
                    self.doubled_ants.append(location.ant.ant)
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 13
        if self.armor - amount <= 0 and self.first:
            bees_win()
        else:
...
def apply_effect(effect, bee, duration):
    ...
    def status_effect(colony):
        status_effect.counter += 1
        if status_effect.counter > duration:
            old_action(colony)
        else:
            new_action(colony)
    bee.action = status_effect
    status_effect.counter = 0
